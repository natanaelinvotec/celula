...
export function montarHistorico(el, { perfil, admin }) {
  ...
  <div class="card"><div class="card-h"><h2>${admin ? 'Histórico de orçamentos' : 'Meus orçamentos'}</h2><span class="cnt" id="hCnt"></span><div class="sp"></div><span class="note">Clique na linha para ver os mnemônicos e marcar como convertido</span></div>
    <div class="card-b tbl-wrap"><table class="tbl"><thead><tr><th>Nº</th><th>Data</th><th>Paciente</th><th>Telefone</th><th>Convênio</th>${admin ? '<th>Atendente</th>' : ''}<th>Exames</th><th class="num">Total</th><th>Status</th></tr></thead><tbody id="hBody"><tr><td colspan="9" class="note">Carregando…</td></tr></tbody></table></div></div>`;
  ...
  function render() {
    ...
    $('hBody').innerHTML = list.map(o => { const s = STATUS[o.status] || [o.status, 'pill']; return `<tr data-id="${o.id}" style="cursor:pointer"><td><b>#${o.numero ?? '—'}</b></td><td>${fmtData(o.criadoEm)}</td><td><b>${escapeHtml(o.paciente || '—')}</b></td><td>${escapeHtml(o.telefone || '—')}</td><td>${escapeHtml(o.convenioNome || o.convenio)}${o.renal ? ' <span class="pill red" style="padding:1px 7px">renal</span>' : ''}</td>${admin ? `<td><span class="av s" style="display:inline-grid;vertical-align:middle;margin-right:6px">${escapeHtml(iniciais(o.atendenteNome))}</span>${escapeHtml(o.atendenteNome || '')}</td>` : ''}<td>${o.qtd ?? o.itens?.length ?? 0}</td><td class="num">${brl(o.total)}</td><td><span class="${s[1]}">${s[0]}</span></td></tr>` +
      (open === o.id ? `<tr><td colspan="9" style="padding:6px 12px 14px"><div class="expand"><div style="width:100%"><span class="note">Mnemônicos para digitar no AutoLAC (ordem do pedido):</span></div><code>${(o.mnemonicos || []).join('  ') || '—'}</code>
        ...
        <div style="width:100%" class="note">${(o.itens || []).map(i => `${escapeHtml(i.nome)}${i.prazoDias != null ? ' · ' + i.prazoDias + ' d.u.' : ''}${i.valor != null ? ' · ' + brl(i.valor) : ''}`).join(' &nbsp;|&nbsp; ')}</div></div></td></tr>` : ''); }).join('') || '<tr><td colspan="9" class="note">Nenhum orçamento no filtro.</td></tr>';
  }
  ...
  $('hBody').addEventListener('click', async e => {
    ...
    if (b?.dataset.conv) { const o = rows.find(r => r.id === b.dataset.conv); try { await D.converterOrcamento(o.id); o.status = 'convertido'; o.convertidoEm = new Date(); o.convertidoPorNome = perfil.nome; render(); toast(`Orçamento #${o.numero} da atendente ${o.atendenteNome} — Convertido com sucesso!`, true); } catch (err) { toast('Erro: ' + err.message); } return; }
